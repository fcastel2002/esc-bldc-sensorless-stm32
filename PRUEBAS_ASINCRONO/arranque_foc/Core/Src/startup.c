...
void generate_sine_tables(uint16_t max_pwm) {
    ...
    const float offset =  amplitude + 0.09f;
    ...
    zero_sine_value = max_pwm/2;
}
SineDriveController sine_ctrl = {
    .phase_step = 2,
    .modulation_index = 0.1f,  // Valor inicial bajo
    .timer_arr =15000// Frecuencia inicial baja (~50 Hz)
};
...
//    }
    ...
    */
    ...
    bool current_dir_u = sin_table_U[idx_u] >= zero_sine_value;
    bool current_dir_v = sin_table_V[idx_v] >= zero_sine_value;
    bool current_dir_w = sin_table_W[idx_w] >= zero_sine_value;
     ...
     // Si current_dir_u es 1, se mantiene IN_U, si es 0, se convierte en 0
     GPIOB->ODR = (GPIOB->ODR & ~IN_U) | (current_dir_u ? IN_U : 0);
     GPIOB->ODR = (GPIOB->ODR & ~IN_V) | (current_dir_v ? IN_V : 0);
     GPIOA->ODR = (GPIOA->ODR & ~IN_W) | (current_dir_w ? IN_W : 0);
    // Actualizar PWM
    __HAL_TIM_SET_COMPARE(&htim1, EN_U, pwm_u);
    __HAL_TIM_SET_COMPARE(&htim1, EN_V, pwm_v);
    __HAL_TIM_SET_COMPARE(&htim1, EN_W, pwm_w);
