    ...
    case USB_REQ_TYPE_CLASS :
      switch (req->bRequest)
      {
        ...
        case CUSTOM_HID_REQ_SET_REPORT:
          ...
          USBD_CtlPrepareRx(pdev, hhid->Report_buf,
                                        ...
                                        uint8_t epnum)
{
  ...
  uint16_t rx_len = (uint16_t)USBD_LL_GetRxDataSize(pdev, epnum);
  ((USBD_CUSTOM_HID_ItfTypeDef *)pdev->pUserData)->OutReport(hhid->Report_buf, rx_len);
...
static uint8_t USBD_CUSTOM_HID_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  {
    ((USBD_CUSTOM_HID_ItfTypeDef *)pdev->pUserData)->OutReport(hhid->Report_buf,
                                                               USBD_CUSTOMHID_OUTREPORT_BUF_SIZE);
