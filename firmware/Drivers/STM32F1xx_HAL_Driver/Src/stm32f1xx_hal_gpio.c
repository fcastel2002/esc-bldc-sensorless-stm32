...
void HAL_GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_Init)
{
  ...
  uint32_t registeroffset; /* offset used during computation of CNF and MODE bits placement inside
                              CRL or CRH register */
  ...
  while (((GPIO_Init->Pin) >> position) != 0x00u) {
    ...
    if (iocurrent == ioposition) {
      ...
      /* Based on the required mode, filling config variable with MODEy[1:0] and CNFy[3:2]
       * corresponding bits */
