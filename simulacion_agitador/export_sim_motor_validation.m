...
stepsPerSample = options.sampleTime / structuralReference.controllerDtSeconds;
if abs(stepsPerSample - round(stepsPerSample)) > 1e-9 || ...
        stepsPerSample < 1 || stepsPerSample > structuralReference.maximumHilSteps
    error("export_sim_motor_validation:UnsupportedStepCount", ...
        "sampleTime requires %g HIL steps; connected firmware supports 1..%g.", ...
        stepsPerSample, structuralReference.maximumHilSteps);
end
...
required = ["schemaVersion", "algorithmVersion", "pwmFrequencyHz", ...
    ...
    "hilStepOperationVersion", "maximumHilSteps"];
for name = required
...
if ~isfield(reference, "supportsDeterministicHil") || ...
        ~islogical(reference.supportsDeterministicHil) || ...
        ~isscalar(reference.supportsDeterministicHil) || ...
        ~reference.supportsDeterministicHil || reference.hilStepOperationVersion ~= 1
    error("export_sim_motor_validation:UnsupportedDeterministicHil", ...
        "Connected firmware does not support deterministic HIL_STEP version 1.");
end
end
